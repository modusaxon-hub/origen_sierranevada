...
COMMENT ON TABLE public.user_roles IS 'Roles del sistema: Administrador, Colaborador, Proveedor, Usuario';

INSERT INTO public.user_roles (name, description) VALUES
    ('Administrador', 'Acceso total al sistema, gestión de usuarios y finanzas.'),
    ('Colaborador', 'Miembros de la Gilda, gestión de pedidos y contenidos.'),
    ('Proveedor', 'Productores de café, gestión de stock propio y fincas.'),
    ('Usuario', 'Cliente final, acceso a catálogo e historial de pedidos.')
...
CREATE TABLE public.payments (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    order_id UUID REFERENCES public.orders(id) ON DELETE CASCADE,
    method TEXT NOT NULL CHECK (method IN ('nequi', 'daviplata', 'pse', 'tarjeta', 'efectivo', 'transferencia')),
...
CREATE POLICY "order_items_read"
    ...
    USING (
        order_id IN (
            ...
            OR auth.uid() IN (SELECT id FROM public.profiles WHERE role_name = 'Administrador')
        )
    );
...
INSERT INTO public.products (
    category, name, price, image_url, stock, available,
    description, story, tags, badge, score, color, mask_type
) VALUES
(
    'cafetal',
